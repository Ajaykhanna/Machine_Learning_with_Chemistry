...
def gaussian_sbatch_template(
    ...
    gaussian_root: str,
    gaussian_exe: str,
    gaussian_library_path: str,
    gaussian_scratch_root: str,
) -> str:
    return f"""#!/bin/csh
...
setenv MY_SCRATCH {gaussian_scratch_root}/${{SLURM_JOBID}}/
mkdir -p ${{MY_SCRATCH}}
setenv GAUSS_SCRDIR ${{MY_SCRATCH}}

setenv g16root "{gaussian_root}"
if ( -e ${{g16root}}/g16/bsd/g16.login ) then
    source ${{g16root}}/g16/bsd/g16.login
endif
setenv GAUSSIAN_EXE "{gaussian_exe}"
if ( "{gaussian_library_path}" != "" ) then
    if ( $?LD_LIBRARY_PATH ) then
        setenv LD_LIBRARY_PATH "{gaussian_library_path}:${{LD_LIBRARY_PATH}}"
    else
        setenv LD_LIBRARY_PATH "{gaussian_library_path}"
    endif
endif
...
$GAUSSIAN_EXE -m={mem_gb}GB -p={cpus} < {com_name} > {log_name}
...
def deploy_gsopt(mol_dir: Path, args: argparse.Namespace) -> bool:
    ...
    (mol_dir / sbatch_name).write_text(gaussian_sbatch_template(
        f"{mol_name}_gsopt",
        com_name,
        log_name,
        err_name,
        ...
        args.gaussian_root,
        args.gaussian_exe,
        args.gaussian_library_path,
        args.gaussian_scratch_root,
    ))
...
def main() -> int:
    ...
    parser.add_argument("--partition", default="shared-spr", help="SLURM partition for generated Gaussian jobs.")
    parser.add_argument("--gaussian-root", default="/usr/projects/cint/Gaussian/g16A03", help="Gaussian root directory containing g16/bsd/g16.login.")
    parser.add_argument("--gaussian-exe", default="g16", help="Gaussian executable name or absolute path.")
    parser.add_argument("--gaussian-library-path", default="", help="Optional Gaussian library path to prepend to LD_LIBRARY_PATH.")
    parser.add_argument("--gaussian-scratch-root", default="/tmp/akhanna2/GAUSSIAN_SCR", help="Root directory for Gaussian scratch subdirectories.")
