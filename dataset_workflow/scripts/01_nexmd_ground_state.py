...
def write_sbatch(
    ...
    content = re.sub(r"#SBATCH --partition=.*", f"#SBATCH --partition={partition}", content)
    content = re.sub(r"^PREFIX=.*$", f"PREFIX=\"${{PREFIX:-{molecule}}}\"", content, flags=re.MULTILINE)
    content = re.sub(r"^NEXMD_ROOT=.*$", f"NEXMD_ROOT=\"${{NEXMD_ROOT:-{nexmd_root}}}\"", content, flags=re.MULTILINE)
    content = re.sub(r"^NEXMD_LIBRARY_PATH=.*$", f"NEXMD_LIBRARY_PATH=\"${{NEXMD_LIBRARY_PATH:-{nexmd_library_path}}}\"", content, flags=re.MULTILINE)
    content = re.sub(r"^NEXMD_EXE=.*$", f"NEXMD_EXE=\"${{NEXMD_EXE:-{nexmd_exe}}}\"", content, flags=re.MULTILINE)
...
def submit_job(sbatch_path: Path, cwd: Path, prepare_only: bool) -> None:
    ...
    while True:
        status = subprocess.run(
            ["sacct", "-j", job_id, "--noheader", "--format=State", "-P"],
            capture_output=True,
            text=True,
        )
        state = ""
        for line in status.stdout.splitlines():
            if line.strip():
                state = line.split("|", 1)[0].strip().split()[0]
                break
        if state:
            print(f"-> NEXMD job {job_id} state: {state}")
        if state in terminal:
            if state != "COMPLETED":
                raise RuntimeError(f"NEXMD job {job_id} ended with {state}")
            return
        time.sleep(20)


def prepare_gs_opt(mol_dir: Path, out_dir: Path, base_dir: Path, prepare_only: bool, partition: str, args: argparse.Namespace) -> None:
    atoms = atoms_from_xyz(Path(args.xyz).resolve()) if args.xyz else atoms_from_gaussian(mol_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    update_input_ceon(base_dir / "gsopt_input.ceon", out_dir / "input.ceon", atoms)
    write_sbatch(base_dir / "standard_nexmd_md.sbatch", out_dir / "submit_gs_opt.sbatch", mol_dir.name, "gs_opt", partition, args.nexmd_root, args.nexmd_exe, args.nexmd_library_path)
    print(f"-> Wrote NEXMD GS optimization input in {out_dir}")
    submit_job(out_dir / "submit_gs_opt.sbatch", out_dir, prepare_only)


def prepare_gs_dyn(mol_dir: Path, out_dir: Path, base_dir: Path, prepare_only: bool, partition: str, args: argparse.Namespace) -> None:
    ...
    write_sbatch(base_dir / "standard_nexmd_md.sbatch", out_dir / "submit_gs_dyn.sbatch", mol_dir.name, "gs_dyn", partition, args.nexmd_root, args.nexmd_exe, args.nexmd_library_path)
...
def main() -> int:
    ...
    parser.add_argument("--partition", default="general", help="SLURM partition for generated NEXMD jobs")
    parser.add_argument("--nexmd-root", default="", help="Optional NEXMD installation root to prepend to PATH.")
    parser.add_argument("--nexmd-exe", default="/usr/projects/ml4chem/akhanna2/softwares/NEXMD/nexmd.exe", help="NEXMD executable path.")
    parser.add_argument("--nexmd-library-path", default="", help="Optional NEXMD library path to prepend to LD_LIBRARY_PATH.")
    parser.add_argument("--xyz", default=None, help="Optional XYZ geometry for NEXMD GS optimization. Defaults to Gaussian optimized geometry.")
    ...
    if args.gs_opt:
        ...
        prepare_gs_opt(mol_dir, out_dir, base_dir, args.prepare_only, args.partition, args)
    else:
        out_dir = Path(args.out_dir).resolve() if args.out_dir else mol_dir / "nexmd" / "gs_dyn"
        prepare_gs_dyn(mol_dir, out_dir, base_dir, args.prepare_only, args.partition, args)
    return 0
