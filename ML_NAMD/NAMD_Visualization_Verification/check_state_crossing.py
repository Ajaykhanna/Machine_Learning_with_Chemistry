...
def get_frame_data(frame_idx, base_dir, n_states=5):
    ...
    try:
        # 1. Coordinates
        coords_data = np.loadtxt(os.path.join(frame_dir, "coords.xyz"), skiprows=2, dtype=str)
...
def main(args):
    ...
    with multiprocessing.Pool(args.cores) as pool:
        for res in tqdm(pool.starmap(get_frame_data, pool_args), total=args.frames, desc="Extracting"):
            if res is not None:
                results.append(res)
    ...
    for t in range(len(valid_frames) - 1):
        ...
        for i in range(args.n_states - 1):
            ...
            if gap_ev < args.threshold_ev:
                ...
                print(f"{frame_num:<8} | S{i+1}-S{j+1:<6} | {gap_ev:<10.4f} | {cross_type}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_dir", type=str, required=True, help="Path containing frame_X directories")
    parser.add_argument("--frames", type=int, default=1000, help="Number of frames to scan")
    parser.add_argument("--n_states", type=int, default=5, help="Number of excited states to track (e.g., 5 for S1-S5)")
    parser.add_argument("--threshold_ev", type=float, default=0.1, help="Energy gap threshold in eV to flag a crossing")
