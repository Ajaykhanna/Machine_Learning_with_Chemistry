...
# This Jupyter Notebook Articles was inspired by the following article. I used this
# articles to understand the basiscs of ML and its application in Chemistry.
...
# selects the molecule in row 1 of the training data, which is hexanal. If you want to select 12-selenazole in row 3 instead,
# change the line of code to read `idx_notCarbonyl=3`.
...
notCarbonyl = train.set_index("name").iloc[
    idx_notCarbonyl, 3:
]  # C!=O Object dataset: X: Frequencies, Y: Intensities, Name: Mol name
...
# Graph Layout
fig.update_layout(
    title="Intensities over frequency",
    title_x=0.5,
...
def df_normalize(df, i=3):
  """
  The `df_normalize` function applies min-max scaling to the specified columns of a DataFrame.
  
  :param df: The parameter `df` is a pandas DataFrame object that represents the data you want to
  normalize
  :param i: The parameter `i` in the `df_normalize` function is the column index for intensities. It
  specifies the starting column index from which the min-max scaling should be applied. In this case,
  it is set to 3, which means the min-max scaling will be applied to columns starting from, defaults
  to 3 (optional)
  ...
  df.iloc[:, i:] = min_max_scaler.fit_transform(
      df.iloc[:, i:].T
  ).T  # Row(Start) --> Cols(Apply Transform) --> Row (End)
...
# equal to 0.
#
# Let's look at the spectra of a few molecules and then choose the threshold.
...
# - The second argument `i` represents the column index where the frequency data start.
# If you don't provide this argument, the function uses a default value of `3`.
# - As an example, if the frequency data in the variable `ex_data` starts in column `4`,
# you would write: `applyThreshold(ex_data,4)`
...
def applyThreshold(dataframe, i=3):
    ...
    """
    dataframe.iloc[:, i:] = np.where(
        (dataframe.iloc[:, i:] < threshold), 0, dataframe.iloc[:, i:]
    )
...
# define a function to split the column containing the label from the columns
# containing the attributes
def splitXY(dataframe, start_X=3, end_X=None, start_Y=2, end_Y=3):
  """
  The function `splitXY` takes a dataframe as input and returns two arrays, X and Y, where X contains
  the normalized intensities of all molecules and Y contains the labels.
  
  :param dataframe: The input dataframe containing the data
  :param start_X: The index of the starting column for the X values in the dataframe. By default, it
  is set to 3, defaults to 3 (optional)
  :param end_X: The end index (exclusive) for the columns of the X matrix. If not specified, it will
  include all columns starting from the start_X index
  :param start_Y: The start_Y parameter is the index of the column where the Y values start in the
  dataframe. It is used to specify the starting column for the Y values, defaults to 2 (optional)
  :param end_Y: The parameter "end_Y" is the ending index (exclusive) of the columns to be included in
  the Y variable. It determines which columns of the dataframe will be used as the Y variable,
  defaults to 3 (optional)
  :return: two variables, X and Y. X is a matrix or list of lists containing the normalized
  intensities of all molecules, and Y is a vector containing the labels of the molecules.
  """
    ...
    Y = (
        Y.values.ravel()
    )  # If .value(), it will be a list of list, ravel makes its a vector
...
# are imbalanced. Here, imbalanced means that there are unequal numbers
...
fig = go.Figure(data=[go.Pie(labels=my_labels, values=data)])
...
# the model might simply learn to ignore the less common ("minority") class. To address
# this possibility, we will use a technique called [SMOTE]
# (https://en.wikipedia.org/wiki/Oversampling_and_undersampling_in_data_analysis#SMOTE)
# which generates new instances of the minority class by interpolating between the existing
# instances. (Note that if the two classes are sufficiently distinct, as is the case here,
# a data balancing step may not be required -- but we'll do it anyway.)
...
# Now the training data are balanced between the two classes.
# We can plot one of the new synthetic carbonyl-containing spectra
# for comparison to a real carbonyl-containing spectrum.
# (The synthetic spectrum will vary each time you run SMOTE.)
# Note that the synthetic spectra are stored at the end of the
# variable X, so any index value greater than the original length
# of the variable train corresponds to a synthetic spectrum.

# index values of a real and synthetic carbonyl (you can change these values to see other spectra)
idx_realCarbonyl = (
    1  # this selects the molecule in row 1 of the training data (hexanal)
)
...
fig.add_trace(
    go.Scatter(x=frequencies, y=spectrum_realCarbonyl, name="Real C=O", mode="markers")
)

fig.add_trace(
    go.Scatter(x=frequencies, y=spectrum_synCarbonyl, name="Syn C=O", mode="markers")
)

# Graph Layout
fig.update_layout(
    title="Real vs Synthetics Samples",
    title_x=0.5,
    xaxis=dict(title="$Frequencies (cm^{-1})$"),
    yaxis=dict(title="Normalized Intensities"),
)
...
# **Decision Tree**
#
# Decision Tree uses a tree-like model of decisions. It models the data in a
# tree structure, in which each leaf node corresponds to a class label and
# attributes are the tree’s internal nodes.
...
# Now that we have trained our machine learning models, we can use them on
# test data. There are a few steps in this process:
# 1. Label Prediction:
# We will use the fitted machine learning models to predict the labels
# (with or without carbonyl) for the test dataset. We will store the predictions
# made by the fitted models in the `Y_pred` list.

# 2. Model Evaluation:
# It's important to see how well the models performed. There are [a few ways]
# (https://en.wikipedia.org/wiki/Evaluation_of_binary_classifiers) to assess the
...
# Finally, we can analyze the error cases, where the model's prediction was wrong.
# A false positive (FP) is an outcome where the model incorrectly predicts the positive
# class. A false negative (FN) is an outcome where the model incorrectly predicts the
# negative class. Sometimes we might be able to understand why the prediction was
# incorrect for a particular molecule by:

# - Looking at the molecular structure. (We will use the `RDkit` module for this task.)
# -Looking at the molecular spectrum.

# ## Test the Decision Tree Model
# Now let's use the fitted Decision Tree model for label prediction.
# Then we'll analyze the performance by displaying the accuracy, sensitivity,
# and specificity.

# use the fitted Decision Tree model to predict the labels (with or without carbonyl)
# for the test dataset
Y_pred = dt_clf.predict(X_test)

# determine the accuracy, sensitivity, and specificity by comparing
...
# Now let's look more carefully at the FPs and FNs. First we need
# to determine which predictions were incorrect and separate them
# into the two types of errors.

# Note that for both the Decision Tree and the Random Forest, we saved
# the predictions in the same variable: `Y_pred`. Thus `Y_pred` currently contains
# the predictions of whichever analysis you ran most recently. If you went straight
# through the file, that would be the Random Forest model.

# Thus to see the FPs and FNs for the Decision Tree model, we will first rerun the prediction
# using the Decision Tree model. If you would like to use the code below to see the FPs and FNs
# for a different model, change the first line below to store the predictions of a different model
# in Y_pred.
...
# go through all predictions to identify the errors and then determine whether
# each one is an FP or FN
...
#
# We can use the `RDkit` library to display the structure of a molecule.
# To identify the molecule, we use the Simplified Molecular Input Line Entry
# System, or [SMILES](https://en.wikipedia.org/wiki/Simplified_molecular-input_line-entry_system).
# SMILES is an unique chemical identifier in the form of an ASCII string. First we will go through
# the lists of FPs and FNs to get the SMILES strings and molecule names.
#
...
img1 = Chem.Draw.MolsToGridImage(
    fp_list,
    molsPerRow=4,
    subImgSize=(200, 200),
...
img2 = Chem.Draw.MolsToGridImage(
    fn_list,
    molsPerRow=4,
    subImgSize=(200, 200),
...
# or FN for inspection. Do you see any spectral features that might explain the error?

# To display the spectrum of a FP in the line below, insert the index value
# of the molecule you want to see from the table above
fp_idx = 78
...
# To display the spectrum of a FN # in the line below, insert the
# index value of the molecule you want to see from the table above
fn_idx = 137
# then display the figure
